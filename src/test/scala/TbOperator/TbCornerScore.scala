...
case class TbCornerScore(config : cornerScoreConfig) extends CornerScore(config){
    ...
    def in(src: String): Unit = {
        fork {
            for (line <- Source.fromFile(src).getLines) {
                for (i <- 0 to 17) {
                    if(i == 17){
                        io.islight #= (BigInt(line.trim.substring(i * 2, i * 2 + 2), 16) == 1)
                    }
                    else {
...
object TbCornerScore extends App {
    ...
    SimConfig.withWave.withConfig(spinalConfig).compile(new TbCornerScore(cornerScoreConfig())).doSimUntilVoid { dut =>
        dut.init
        //dut.io.start #= true
        dut.clockDomain.waitSampling(10)
        val path = "F:\\TestData\\OpencvData\\CornerScore"
