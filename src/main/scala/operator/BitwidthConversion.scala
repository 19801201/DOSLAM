...
case class BitwidthConversionConfig() {
    val DATA_WIDTH = 8
    val DATA_STREAM_WIDTH_IN = DATA_WIDTH * 8
    val DATA_STREAM_WIDTH_OUT = DATA_WIDTH * 10
    //输入像素的位宽
    val SIZE_WIDTH = 11
    //输入大小的位宽
    val WINDOWS_SIZE = 2
...
class BitwidthConversion(config: BitwidthConversionConfig) extends Component {
    val io = new Bundle {
        ...
        val colNumIn = in UInt (config.SIZE_WIDTH bits)//输入大小应该包含两部分，第一部分是输入的次数，第二部分是后八位代表列数，n代表有n个有效数
    }
    //状态机 状态机的跳转
    val fsm = new StateMachine {
        ...
        //一次输入八个数据这样可以更快的计算，
        val colCnt = WaCounter(io.sData.fire, config.SIZE_WIDTH - 3, io.colNumIn(config.SIZE_WIDTH - 1 downto 3) - 1) //从最大值开始
    ...
    //判断末尾有效数据位数
    fsm.dataInvalid := (BWCcount.count > io.rowNumIn(2 downto 1)) || !BWCcount.count.orR
    /*
    0 : 0
    1 : 8  0到7位还没有被接收
    2 : 6  2到7位还没有被接收
    ...
    switch(BWCcount.count) { //fire的时候，sdata,边界条件如何处理？？？，
        ...
        is(1) { //已经接收了一个数据，加上sData给的可以拼成一个数据，8
            io.mData.payload :=  sDataPayTemp ## io.sData.payload(63 downto 48)
            io.mData.valid := mValid
            //1、上层valid拉高 2、这里存在最后的有效数据没有传递->有效数据中的无效数据->有效数据中的有数据
        }
        is(2) { //已经接收了两个数据，加上sData给的可以拼成一个数据，
            io.mData.payload := sDataPayTemp.asBits(47 downto 0) ## io.sData.payload(63 downto 32)
              io.mData.valid := mValid
        }
        is(3) {
            io.mData.payload := sDataPayTemp.asBits(31 downto 0) ## io.sData.payload(63 downto 16)
              io.mData.valid := mValid
        }
        is(4) { //已经接收了四个数据，加上sData给的可以拼成一个数据，sData的数据变为废数，不在需要，2
            io.mData.payload := sDataPayTemp.asBits(15 downto 0) ## io.sData.payload(63 downto 0)
              io.mData.valid := mValid
        }
        default {
            io.mData.valid := False
